...
A (WIP) minimal J1939 implementation, based on the information found in Wilfried Voss' [A Comprehensible Guide to J1939](https://copperhilltech.com/a-comprehensible-guide-to-j1939/).
...
There are a few CMake variables that can be enabled for controlling the build. This repo contains a demo project that uses the Linux SocketCAN API for testing the library. You can build this by setting the variable `J1939_DEMO` (e.g.: when configuring, pass the option `-DJ1939_DEMO=1` to CMake). There's options to disable the compilation of the address claim and transport protocol translation units, if they're not needed.
